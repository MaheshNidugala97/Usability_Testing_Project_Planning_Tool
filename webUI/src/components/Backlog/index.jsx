...
export default function Backlog() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("https://projectpilotbe-9799004ffbe5.herokuapp.com/api/issues");
        if (!response.ok) {
  ...
  const handleDragEnd = async (result) => {
    ...
    try {
      ...
      await axios.patch(`https://projectpilotbe-9799004ffbe5.herokuapp.com/api/issues/${taskId}`, { status: newStatus });
    } catch (error) {
